...
const TestimonialsSlider = styled(Slider)`
  ${tw`flex mt-8 mx-auto max-w-xs sm:max-w-xl lg:max-w-5xl text-left bg-white rounded-lg text-gray-900`}
  .slick-track {
    ${tw`flex!`}
  }
  .slick-slide {
...
`;
const Testimonial = tw.div`px-1 py-1 sm:px-1 sm:py-1 focus:outline-none flex! rounded-lg justify-between h-full`
...
const BoxLeft = tw.div `
  flex
  w-2/5
  mr-2
`
const BoxRight = tw.div `
  flex
  w-3/5
  ml-2
`
const Boxavatar = tw.div `
    w-full
    h-16
    rounded-lg
    bg-gray-200
`
const Imageavata = tw.img `

`

export default ({
  subheading = "",
  Imagescr = "../../images/user-thai.jpg",
      ...
      <HeadingContainer>
        ...
        <TestimonialsSlider >
            <Testimonial>
                <BoxLeft>
                    <Boxavatar>
                        <Imageavata src={Imagescr}/>
                    </Boxavatar>
                </BoxLeft>
                <BoxRight>Thái</BoxRight>
            </Testimonial>
        </TestimonialsSlider>
