...
const DownvoteFeatures = ({id}) => {
    ...
    temp[id] = { value: downvotenumber, set: setDownvotenumber}
      ...
      <div>
        <div>
          <span style={{display:"flex"}}>
            <span style={{display:"flex"}}>
              <DownvoteIcon />
                <span style={{margin:"0 5px"}}> {data.downvote.status[id].value} </span>
            </span>
        ...
        <div className="up-down-button">
          <InteractButton onClick={onClickDownvote}>Downvote </InteractButton> 
        </div>
      </div>
