...
const UpvoteFeatures = ({id}) => {
    ...
    temp[id] = { value: upvotenumber, set: setUpvotenumber}
      ...
      <div>
        <div>
          <span style={{display:"flex"}}>
            <span style={{display:"flex"}}>
              <UpvoteIcon />
                <span style={{margin:"0 5px"}}> {data.upvote.status[id].value} </span>
            </span>
        ...
        <div className="up-down-button">
          <InteractButton onClick={onClickUpvote}>Upvote </InteractButton> 
        </div>
      </div>
