...
export default ({ roundedHeaderButton = false, logoLink, links, className, collapseBreakpointClass = "lg" }) => {
  ...
  const onClickTruyNow = async (e) => {
    ...
    if (!window.keplr) {
      ...
      axios.get(`https://aiclub.uit.edu.vn/namnh/slearn/be/user/check?id=${id}`)
        .then(response => {
          ...
          if (response.data.is_exist) {
            ...
            axios.post(`https://aiclub.uit.edu.vn/namnh/slearn/be/user/create`, data_create)
              .then((response) => {
  ...
  const defaultLinks = [
    <NavLinks key={1}>
      ...
      <DropdownMenu value={showMenu}>
        ...
        <DropdownItem>
          <Link to="/chatbot/slearn">Chatbot</Link>
        </DropdownItem>
      </DropdownMenu>
    </DropdownContainer>
      ...
      <Link onClick={onClickTruyNow}>
        <PrimaryLink css={roundedHeaderButton && tw`rounded-full`} >Try a beta</PrimaryLink>
      </Link>
    </NavLinks>
  ];
